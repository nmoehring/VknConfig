...
namespace EngineCore
{
                                         ...
                                         std::function<void(int, int)> callback)
  {
    ...
    for (int i = 0; i < document->textures.Size(); ++i)
    {
      //.submit() -> submit_task() as of v4.0.0
      results_.emplace_back(
          pool.submit_task(
              [pathCurrent, document, i, outModel, callback]
              { return imageDataAsync(pathCurrent, document, i, document->textures[i].imageId, outModel, modelId, callback); }));
                              ...
                              bool makeBuffersSuitableForAccelStruct)
  {
    ...
    buffers.emplace_back(context.createBuffer(
        ...
        VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
#endif
            VK_BUFFER_USAGE_TRANSFER_DST_BIT | (makeBuffersSuitableForAccelStruct ? VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR : 0) |
            VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
    ...
    buffers.emplace_back(context.createBuffer(
        ...
        VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
            ...
            VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
            VK_BUFFER_USAGE_INDEX_BUFFER_BIT | (makeBuffersSuitableForAccelStruct ? VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR : 0) |
